...
router.post('/register', (req, res) => {
  try {
    ...
    const existing = queryOne('SELECT id FROM joueur WHERE pseudo = ?', [pseudo]);
    if (existing) return res.status(409).json({ error: 'Ce pseudo est déjà pris' });

    const hash = bcrypt.hashSync(password, 10);
    const count = queryOne('SELECT COUNT(*) as c FROM joueur');
    const role = count.c === 0 ? 'admin' : 'membre';

    const result = run('INSERT INTO joueur (pseudo, password_hash, role) VALUES (?, ?, ?)', [pseudo, hash, role]);
    run('INSERT OR IGNORE INTO frequence (joueur_id, raids_par_semaine_max) VALUES (?, 3)', [result.lastId]);
...
router.post('/login', (req, res) => {
  ...
  const user = queryOne('SELECT * FROM joueur WHERE pseudo = ?', [pseudo]);
...
router.get('/me', authMiddleware, (req, res) => {
  const user = queryOne('SELECT id, pseudo, role, created_at FROM joueur WHERE id = ?', [req.user.id]);
  if (!user) return res.status(404).json({ error: 'Utilisateur non trouvé' });
  const freq = queryOne('SELECT raids_par_semaine_max FROM frequence WHERE joueur_id = ?', [req.user.id]);
  res.json({ ...user, raids_par_semaine_max: freq?.raids_par_semaine_max ?? 3 });
});

router.put('/frequence', authMiddleware, (req, res) => {
  const { raids_par_semaine_max } = req.body;
  const existing = queryOne('SELECT id FROM frequence WHERE joueur_id = ?', [req.user.id]);
  if (existing) {
    run('UPDATE frequence SET raids_par_semaine_max = ? WHERE joueur_id = ?', [raids_par_semaine_max, req.user.id]);
  } else {
    run('INSERT INTO frequence (joueur_id, raids_par_semaine_max) VALUES (?, ?)', [req.user.id, raids_par_semaine_max]);
  }
  res.json({ ok: true });
});

router.get('/membres', authMiddleware, (req, res) => {
  try {
    ...
    for (const m of membres) {
      const main = queryOne("SELECT nom, classe, role FROM personnage WHERE joueur_id = ? AND type = 'Main' LIMIT 1", [m.id]);
...
router.get('/profil/:id?', authMiddleware, (req, res) => {
  try {
    const targetId = req.params.id ? Number(req.params.id) : req.user.id;
    const user = queryOne('SELECT id, role, created_at FROM joueur WHERE id = ?', [targetId]);
    if (!user) return res.status(404).json({ error: 'Utilisateur non trouvé' });

    const freq = queryOne('SELECT raids_par_semaine_max FROM frequence WHERE joueur_id = ?', [targetId]);
    user.raids_par_semaine_max = freq?.raids_par_semaine_max ?? 3;

    const chars = queryAll('SELECT * FROM personnage WHERE joueur_id = ?', [targetId]);
