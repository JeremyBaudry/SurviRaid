...
router.get('/', (req, res) => {
  ...
  const validations = queryAll('SELECT personnage_id, instance, statut FROM validation_stuff');
  const validMap = {};
  validations.forEach(v => { validMap[`${v.personnage_id}|${v.instance}`] = v.statut; });

  const activeDays = queryAll('SELECT jour, actif FROM jour_actif');
  ...
  `);
  const joueursAvecPrefs = queryAll(`
