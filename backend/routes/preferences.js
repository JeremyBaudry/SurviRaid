...
router.put('/', authMiddleware, async (req, res) => {
  try {
    ...
    const ownedChars = (await queryAll('SELECT id FROM personnage WHERE joueur_id = ?', [req.user.id])).map(c => c.id);
    const activeDays = (await queryAll('SELECT jour FROM jour_actif WHERE actif = 1')).map(d => d.jour);

    for (const charId of ownedChars) {
      await run('DELETE FROM preference_raid WHERE personnage_id = ?', [charId]);
    }
    ...
    for (const pref of preferences) {
      ...
      await run(
        'INSERT OR IGNORE INTO preference_raid (personnage_id, instance, jour, statut) VALUES (?, ?, ?, ?)',
...
router.get('/jours-actifs', async (req, res) => {
  try {
    const jours = await queryAll(`SELECT * FROM jour_actif ORDER BY
